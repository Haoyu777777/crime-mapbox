...
$(function () {
    ...
    let crimeType = ["Accident - w/out Injuries",
...
// layers on the map
let layers = ["neighbor-fill", "neighbor-count", "unclustered-point", "clusters", "cluster-count"];
...
map.on('click', 'neighbor-fill', function (e) {
    ...
    let startDate = formatDate(new Date(new Date($('#start').val()).getTime() + 86400000));
    let endDate = formatDate(new Date(new Date($('#end').val()).getTime() + 86400000));
    let dateRange = startDate + " -- " + endDate;

    new mapboxgl.Popup()
        .setLngLat(e.lngLat)
        .setHTML('<h3>Neighbor: ' + e.features[0].properties.NAME + '</h3><p>Crime record: ' + count + ' </p><p>Date range: ' + dateRange + '</p>')
        .addTo(map);
});
...
map.on('click', 'clusters', function (e) {
    ...
    map.getSource('crime_data').getClusterExpansionZoom(clusterId, function (err, zoom) {
        if (err) return;
...
function createPopUp(currentFeature) {
    ...
    let popup = new mapboxgl.Popup({
        ...
        .setHTML('<h3>Offense: ' + currentFeature.properties.Offense + '</h3><p>ID: ' + currentFeature.properties.IncidentID + ' </p><p>Location: ' + currentFeature.properties.FullAddress + '</p><p>Time: ' + localTimeString + '</p>')
        .addTo(map);
}
...
function buildList(dataList) {
    ...
    if (dataList.length) {
        ...
        for (i = 0; i < dataList.length; i++) {
            ...
            details.innerHTML = prop.FullAddress + ' on ' + new Date(prop.DateReported).toDateString();
...
function swapStyle(styleID) {
    ...
    d3.json(`https://api.mapbox.com/styles/v1/mapbox/${styleID}?access_token=${mapboxgl.accessToken}`, (newStyle) => {
        ...
        let labelIndex = newStyle.layers.findIndex((el) => { // find the index of where to insert our layers to retain in the new style
            return el.id == 'waterway-label';
        });
        let appLayers = currentStyle.layers.filter((el) => { // app layers are the layers to retain, and these are any layers which have a different source set
